...
interface Allergen {
  id: string;        // id stabile (UUID-ish), invariato anche se cambia il code
  code: string;      // ciò che si vede al cliente: "1", "1b", "12a", ecc.
  name: string;
  description: string;
}
...
interface AppData {
  ...
  theme?: ThemeColors;
  allergens?: Allergen[];
  allergensEnabled?: boolean; // mostra/nascondi gli allergeni nel menù pubblico
  socials: {
...
const DEFAULT_ALLERGENS: Allergen[] = [
  { id: 'a1',  code: '1',  name: 'Arachidi e derivati', description: 'snack confezionati, creme e condimenti in cui vi sia anche in piccole dosi' },
  { id: 'a2',  code: '2',  name: 'Frutta a guscio', description: 'mandorle, nocciole, noci comuni, noci di acagù, noci pecan, anacardi e pistacchi' },
  { id: 'a3',  code: '3',  name: 'Latte e derivati', description: 'ogni prodotto in cui viene usato il latte, yogurt, biscotti, torte, gelato e creme varie' },
  { id: 'a4',  code: '4',  name: 'Molluschi', description: 'canestrello, cannolicchio, capasanta, cozza, ostrica, patella, vongola, telline ecc' },
  { id: 'a5',  code: '5',  name: 'Pesce', description: 'prodotti alimentari in cui è presente il pesce, anche se in piccole percentuali' },
  { id: 'a6',  code: '6',  name: 'Sesamo', description: 'semi interi usati per il pane, farine anche se lo contengono in minima percentuale' },
  { id: 'a7',  code: '7',  name: 'Soia', description: 'prodotti derivati come: latte di soia, tofu, spaghetti di soia o simili' },
  { id: 'a8',  code: '8',  name: 'Crostacei', description: 'marini e d\'acqua dolce, gamberi, scampi, aragoste, granchi e simili' },
  { id: 'a9',  code: '9',  name: 'Glutine', description: 'cereali, grano, segale, orzo, avena, farro, Kamut, inclusi ibridati derivati' },
  { id: 'a10', code: '10', name: 'Lupini', description: 'presenti in cibi vegan sottoforma di arrosti, salamini, farine e similari' },
  { id: 'a11', code: '11', name: 'Senape', description: 'si può trovare nelle salse e nei condimenti, specie nella mostarda' },
  { id: 'a12', code: '12', name: 'Sedano', description: 'sia in pezzi che all\'interno di preparati per zuppe, salse e concentrati vegetali' },
  { id: 'a13', code: '13', name: 'Anidride solforosa e solfiti', description: 'cibi sott\'aceto, sottolio e in salamoia, marmellate, funghi secchi, conserve ecc' },
  { id: 'a14', code: '14', name: 'Uova e derivati', description: 'uova e prodotti che le contengono come maionese, emulsionati e pasta all\'uovo' },
];
...
export default function Dashboard() {
  ...
  const migrateData = (raw: any): AppData => {
    ...
    return {
      ...
      allergens: Array.isArray(raw.allergens) && raw.allergens.length ? raw.allergens : DEFAULT_ALLERGENS,
      allergensEnabled: typeof raw.allergensEnabled === 'boolean' ? raw.allergensEnabled : true,
  ...
  const addAllergen = () => {
    ...
    // Trova il prossimo numero libero come default
    const usedNums = new Set(allergens.map(a => parseInt(a.code, 10)).filter(n => !isNaN(n)));
    let nextNum = 1;
    ...
    setData(prev => ({ ...prev, allergens: [...(prev.allergens || []), newAllergen] }));
  };

  const updateAllergen = (id: string, field: keyof Allergen, value: string) => {
    setData(prev => ({
      ...prev,
      allergens: (prev.allergens || []).map(a => a.id === id ? { ...a, [field]: value } : a)
    }));
  };

  const removeAllergen = (id: string) => {
    if (!confirm('Eliminare questo allergene? Verrà rimosso anche dai piatti che lo contengono.')) return;
    setData(prev => {
      // Rimuove anche i riferimenti dagli items del menu
      const newMenu: Record<string, MenuItem[]> = {};
      Object.keys(prev.menu).forEach(catKey => {
        newMenu[catKey] = prev.menu[catKey].map(it => ({
          ...it,
          allergenIds: (it.allergenIds || []).filter(aid => aid !== id)
        }));
      });
      return {
        ...prev,
  ...
  const moveAllergen = (id: string, direction: -1 | 1) => {
    setData(prev => {
      ...
      if (idx < 0 || newIdx < 0 || newIdx >= list.length) return prev;
      [list[idx], list[newIdx]] = [list[newIdx], list[idx]];
      return { ...prev, allergens: list };
    });
  };

  const toggleItemAllergen = (cat: string, itemId: string, allergenId: string) => {
    setData(prev => ({
      ...prev,
      menu: {
        ...prev.menu,
        [cat]: prev.menu[cat].map(it => {
          if (it.id !== itemId) return it;
          const current = it.allergenIds || [];
          const next = current.includes(allergenId)
            ? current.filter(a => a !== allergenId)
            : [...current, allergenId];
          return { ...it, allergenIds: next };
        })
      }
    }));
  };
  ...
  const resetAllergens = () => {
    if (!confirm('Ripristinare i 14 allergeni predefiniti? I riferimenti nei piatti potrebbero perdersi.')) return;
    setData(prev => ({ ...prev, allergens: DEFAULT_ALLERGENS }));
  };
            ...
            <section className="bg-white rounded-xl border border-neutral-200 p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4 mb-2 flex-wrap">
                ...
                <label className="flex items-center gap-2 text-xs text-neutral-600">
                  <input
                    type="checkbox"
                    checked={data.allergensEnabled !== false}
                    onChange={e => setData(prev => ({ ...prev, allergensEnabled: e.target.checked }))}
                    className="w-4 h-4"
                  />
                  Mostra allergeni nel menù pubblico
                </label>
              </div>
              <p className="text-xs text-neutral-500 mb-4">
                Numera (o etichetta) gli allergeni e poi assegnali ai singoli piatti dalla sezione "Gestione Menu". I numeri appariranno come pillole accanto al nome del piatto.
              </p>

              <div className="space-y-2 mb-4">
                {(data.allergens || []).map((allergen, idx) => (
                  <div key={allergen.id} className="flex items-start gap-2 bg-neutral-50 border border-neutral-200 rounded-lg p-3">
                    <div className="flex flex-col pt-1">
                      <button onClick={() => moveAllergen(allergen.id, -1)} disabled={idx === 0} className="p-0.5 text-neutral-400 hover:text-pizza-dark disabled:opacity-30"><ArrowUp size={14} /></button>
                      <button onClick={() => moveAllergen(allergen.id, 1)} disabled={idx === (data.allergens || []).length - 1} className="p-0.5 text-neutral-400 hover:text-pizza-dark disabled:opacity-30"><ArrowDown size={14} /></button>
                    </div>
                    <input
                      value={allergen.code}
                      onChange={e => updateAllergen(allergen.id, 'code', e.target.value)}
                      placeholder="N°"
                      className="w-14 sm:w-16 bg-white border border-neutral-300 rounded px-2 py-2 text-sm font-bold text-center text-pizza-red"
                      maxLength={6}
                    />
                    <div className="flex-1 min-w-0 space-y-2">
                      <input
                        value={allergen.name}
                        onChange={e => updateAllergen(allergen.id, 'name', e.target.value)}
                        placeholder="Nome (es. Glutine)"
                        className="w-full bg-white border border-neutral-300 rounded px-3 py-2 text-sm font-bold"
                      />
                      <textarea
                        value={allergen.description}
                        onChange={e => updateAllergen(allergen.id, 'description', e.target.value)}
                        placeholder="Descrizione (es. cereali, grano, segale...)"
                        rows={2}
                        className="w-full bg-white border border-neutral-300 rounded px-3 py-2 text-xs resize-none"
                      />
                    </div>
                    <button
                      onClick={() => removeAllergen(allergen.id)}
                      title="Elimina allergene"
                      className="p-1.5 text-neutral-500 hover:text-red-500 hover:bg-neutral-200 rounded transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 border-t border-neutral-200 pt-4">
                <button onClick={addAllergen} className="flex-1 bg-pizza-dark hover:bg-black text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2">
                  <Plus size={16} /> Aggiungi allergene
                </button>
                <button onClick={resetAllergens} className="text-xs text-neutral-500 hover:text-pizza-red font-bold px-3 py-2">
                  ↺ Ripristina i 14 predefiniti
                </button>
              </div>
            </section>
            ...
            >
              <div className="h-full flex flex-col no-scrollbar" style={{ background: data.theme?.cream }}>
                ...
                <div className="p-6 flex-1">
                  ...
                  {/* Legenda allergeni (preview) */}
                  {data.allergensEnabled !== false && (data.allergens || []).length > 0 && (
                    <details className="mt-10 border rounded-xl overflow-hidden" style={{ borderColor: `${data.theme?.dark}20` }}>
                      <summary className="cursor-pointer px-5 py-4 font-bold text-sm flex items-center gap-2" style={{ color: data.theme?.dark, background: `${data.theme?.primary}0d` }}>
                        <AlertTriangle size={16} style={{ color: data.theme?.primary }} /> Legenda Allergeni
                      </summary>
                      <ul className="divide-y" style={{ borderColor: `${data.theme?.dark}10` }}>
                        {(data.allergens || []).map(a => (
                          <li key={a.id} className="px-5 py-3 text-sm flex gap-3">
                            <span className="font-black shrink-0 w-8 text-center rounded px-1.5 py-0.5 text-xs self-start" style={{ background: data.theme?.primary, color: '#fff' }}>{a.code}</span>
                            <div>
                              <p className="font-bold" style={{ color: data.theme?.dark }}>{a.name}</p>
